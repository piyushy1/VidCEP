...
public class VGGModelPrediction {
	...
	private VGGModelPrediction() {
		try {

			//preTrained = (ComputationGraph) tinyYOLOZoo.initPretrained();
			preTrained = (ComputationGraph)VGG16.builder().build().initPretrained();
	...
	private static final VGGModelPrediction INSTANCE = new VGGModelPrediction();

	public static VGGModelPrediction getINSTANCE() {
		return INSTANCE;
	}
...
/*		FAST("Real-Time but low accuracy", 224, 224, 7, 7),
		...
		if (newBoundingBOx) {
			...
			INDArray indArray = prepareImage(file, 224, 224);
            if (indArray == null) {
		...
		//imshow(winName, file);
		//imwrite("C:\\Users\\piyush\\Desktop\\test\\test1\\" + i + ".png", file);
		return graph;
